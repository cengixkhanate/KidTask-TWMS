...
public class Parent extends Person{
    ...
    public EndOnlyTask addEndOnlyTask(String[] tasks){
        ...
        return new EndOnlyTask(taskID, taskAssigner, title, description, taskReward, deadline);
    ...
    public StartEndTask addStartEndTask(String[] tasks){
        ...
        int taskReward = Integer.parseInt(tasks[7]);
        return new StartEndTask(taskID, taskAssigner, title, description, taskReward, startTime, endTime);
    }
    ...
    public void approveWish(String id,String requiredLevel, LinkedList<Wish> wishObjectList) {
        for(Wish wish : wishObjectList){
            if(wish.wishID.equals(id)){
                wish.approveThisWish();
                wish.setRequiredLevelWhileApprovingRejecting(requiredLevel);
            }
        }
    }
    public void rejectWish(String id,String requiredLevel, LinkedList<Wish> wishObjectList) {
        for(Wish wish : wishObjectList){
            if(wish.wishID.equals(id)){
                wish.rejectThisWish();
                wish.setRequiredLevelWhileApprovingRejecting(requiredLevel);
